...
class _CaseAnalysisScreenState extends ConsumerState<CaseAnalysisScreen> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Case Analysis failed to load', style: TextStyle(fontWeight: FontWeight.w900)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () => ref.read(caseScenariosProvider.notifier).refresh(),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const Text('Case Analysis', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
                      const Spacer(),
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: _submitting ? null : () => _handleChoice(current, CaseChoice.left),
                          icon: const Icon(Icons.arrow_back),
                          label: Text(current.leftText, maxLines: 1, overflow: TextOverflow.ellipsis),
                        ),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: ElevatedButton.icon(
                          onPressed: _submitting ? null : () => _handleChoice(current, CaseChoice.right),
                          icon: const Icon(Icons.arrow_forward),
                          label: Text(current.rightText, maxLines: 1, overflow: TextOverflow.ellipsis),
  ...
  Future<void> _handleChoice(CaseScenario scenario, CaseChoice choice) async {
    ...
    try {
      await ref.read(caseScenariosProvider.notifier).submitChoice(
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      ...
      background: Container(
        ...
        child: const Icon(Icons.arrow_forward, color: Color(0xFF16A34A), size: 36),
      ),
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Scenario', style: TextStyle(color: Color(0xFF6B7280), fontWeight: FontWeight.w800)),
            ...
            const SizedBox(height: 16),
            const Text('Swipe left or right to answer.',
